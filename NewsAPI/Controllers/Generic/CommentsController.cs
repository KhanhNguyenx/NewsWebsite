...
namespace NewsAPI.Controllers
{
    ...
    public class CommentsController : ControllerBase
    {
        ...
        private readonly IGenericServive<Comment> _genericServive;
        private readonly IMapper _mapper;

        public CommentsController(IGenericServive<Comment> genericServive, IMapper mapper)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<Comment>> GetFull(int id)
        {
            return await _genericServive.GetAsync(id);
        }
        [HttpPost]
        public async Task<ActionResult<CommentDTO>> Create(CommentDTO model)
        {
            Expression<Func<Comment, int>> filter = (x => x.Id);
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<CommentDTO>>> GetList()
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<CommentDTO>>> Search(string txtSearch)
        {
            Expression<Func<Comment, bool>> filter;
            filter = a => a.Status != -1 /*&& (a.CommentDate!.Contains(txtSearch) || a.Slug!.Contains(txtSearch))*/;
        ...
        [HttpPut]
        public async Task<ActionResult<CommentDTO>> Update(CommentDTO model)
        {
